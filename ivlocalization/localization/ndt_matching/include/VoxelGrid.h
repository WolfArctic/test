...
/*
	void setLeafSize(float voxel_x, float voxel_y, float voxel_z) {
		...
		voxel_y_ = voxel_y;
		voxel_z_ = voxel_z;
fprintf(stderr, "voxel_x is %f\n", voxel_x);
fprintf(stderr, "voxel_y is %f\n", voxel_y);
fprintf(stderr, "voxel_z is %f\n", voxel_z);
	}
...
*/
	...
	//Get the centroid list.
	double *getCentroidList();

	//Get the covariance list.
	double *getCovarianceList();

	//Get the pointer to the inverse covariances list
	double *getInverseCovarianceList();

	int *getPointsPerVoxelList();
