...
map_loader::map_loader(ros::NodeHandle n){
    ...
    sub_status = n.subscribe("ndt_status", 10, &map_loader::callback_status, this);
...
bool map_loader::checkIndex(int input, int center_x, int center_y, int row, int col){
...
void map_loader::mergeAllCloud(){
    ...
    for(unsigned int i = 0; i < findSubFile(maps_); ++ i){
      ...
      InCloud->insert(InCloud->end(), cloudTemp->begin(), cloudTemp->end());
    }
    pcl::toROSMsg(*InCloud, outPointCloud);
    outPointCloud.header.frame_id = std::string("map");
    maps_pub.publish(outPointCloud);
    if(InCloud->size() > 1){
    pcl::io::savePCDFileASCII("/home/elbert/data/avos_test/all.pcd", *InCloud);
    } else {
        ROS_ERROR("The mergecloud is empty!!");
        return;
    }
}

void map_loader::divCloud(){
    int map_size_x = (int)(max_x - min_x) / map_length +1;
    int map_size_y = (int)(max_y - min_y) / map_length +1;
    pcl::PointCloud<pcl::PointXYZI> temp_pcds[map_size_x * map_size_y];
    for(auto i:*cloud){
        int x = (int)(i.x - min_x) / map_length;
        int y = (int)(max_y - i.y) / map_length;
        if((x + y * map_size_x) < (map_size_x * map_size_y))
        temp_pcds[x + y * map_size_x].push_back(i);
    }
    for(unsigned int j = 0; j < map_size_x * map_size_y; j ++){
        std::stringstream ss;
        std::string test = maps_;
        ss << setw(map_bit) << setfill('0') << std::right << j;
        test += ss.str() + ".pcd";
        if(temp_pcds[j].size() > 1)
            pcl::io::savePCDFileASCII(test, temp_pcds[j]);
    }
    ROS_INFO_STREAM("The Map Dividing has done!!!");
}
...
void map_loader::publish_adjacent_pcd(int numX, int numY) {
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr outCloud = boost::make_shared<pcl::PointCloud<pcl::PointXYZI>>();
...
void map_loader::publish_pcd(ivlocmsg::ivmsglocpos loc_input){
    if(map_divide_flag && pcd_size > 0 && !pos_flag && loc_input.lon > 100 && loc_input.lat > 30 
    ...
    else if(!map_divide_flag && !pos_flag && fabs(ros::Time::now().toSec() - last_gps_time) > 3){
...
{
    status_ = *msg;
    if(status_.score > 0 && status_.score < 1)
        initial_flag = true;
}
...
int main(int argc, char **argv)
{
    ...
    //loading_map.mergeAllCloud();
    //loading_map.divCloud();
