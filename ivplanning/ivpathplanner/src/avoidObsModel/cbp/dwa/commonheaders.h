...
struct Site {
    Site()  {}
    Site(double xx, double yy) : x(xx), y(yy) {}
    ~Site() {}
    ...
    struct Compare {
        bool operator()(const Site &l, const Site &r) {
            if (std::fabs(l.x-r.x) < MIN_) return l.y < r.y;
            else return l.x < r.x;
        }
    };
};
...
using blind_tree  = std::map<Site, ros::Time, Site::Compare>;
