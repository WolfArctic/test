...
std::vector<sFreeCircle> sehs::LookAheadDistance(sKinodynamic current)
{
    ...
    {
        distance_threshold = 2 * current.speed - 1.0;
    }
    else
    {
    ...
    {
        temp_heuristic_index = heuristic_index;
    }
    //std::cout<<temp_heuristic_index<<"of"<<circle_path.size()<<std::endl;

    for(int i = temp_heuristic_index + 1; i < circle_path.size(); ++i)
    {
        choose_circle_id = i;
    ...
    if (choose_circle_id < temp_heuristic_index + 2)
    {
        choose_circle_id = temp_heuristic_index + 2;
    }
    if (choose_circle_id >= circle_path.size())
    {
        choose_circle_id = circle_path.size() - 1;
    }

    rtn.push_back(circle_path.at(choose_circle_id).first);
    return rtn;
}
...
ivpathplanner::path rawpath, sortedTree sortedobjs)
{
    ...
    if (true == get_goal_status)
    {
        ...
        for (auto a : trajectory)
        {
            ...
            temp.x = a.x;
            temp.y = a.y;
            temp.angle = a.theta * 180.0 / M_PI;
            temp.velocity = -88.0;
            ivpath.points.push_back(temp);  
        }
        visdwa_marker->publishobstacle(road, 4, 0.05);
