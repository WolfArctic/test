...
const std::vector<std::pair<sFreeCircle, float> >& circle_array)
{
    ...
    for (int i = -1; i <= 6; ++i)
    {
        ...
        distance = std::hypot(current_status.x - circle_array.at(index).first.center.x, 
                                                      current_status.y - circle_array.at(index).first.center.y);

        if (distance <= circle_array.at(index).first.radius)
        {
            if (minus_distance > distance){
                minus_distance = distance;
                heuristic_index = index;
            }
        }
    }
    return std::forward_as_tuple(heuristic_index, distance);
}
...
sehs::PrimitiveActions(sKinodynamic currentvs, float step_length)
{
    ...
    else
    {
        ...
        for (int i = -speed_num; i <= speed_num; ++i)
        {
            ...
            aim_steering = (aim_angle - currentvs.steeringAngle) / controlCycle;
            if (aim_steering > positive_max_steering_velocity)
            {
                aim_steering = positive_max_steering_velocity;
            }
            else if (aim_steering < negetive_max_steering_velocity)
            {
                aim_steering = negetive_max_steering_velocity;
            }

            divide_num = steering_num ;//* (steering_num + 1) / 2;
            positive_steering = (positive_max_steering_velocity - aim_steering) / divide_num;
            negetive_steering = (negetive_max_steering_velocity - aim_steering) / divide_num;
            ...
            for (int j = -steering_num; j <= steering_num; ++j)
            {
                // if (j <= 0)
                // {
                //     steering_velocity = aim_steering + negetive_steering * (-j);// * (-j + 1) / 2;
                // }
                // else
                // {
                //     steering_velocity = aim_steering + positive_steering * j;// * (j + 1) / 2;
                // }
...
bool sehs::HeuristicSearch(sKinodynamic start, sKinodynamic goal, std::vector<sKinodynamic>& trajectory)
{
    ...
    while(!frontier.elements.empty())
    {
        ...
        {
            //This is how the bubble calculates the angle
            ...
            // }
            //This is how MIT calculates
            aim_angle = 0;//ForwardDrive(current , LookAheadDistance(current));
        ...
        for (auto next : PrimitiveActions(current, step))
        {
            ...
            int temp_index = 0;
            sKinodynamic tempvs;
            int tra_size = next_trajectory.size() ;
            if (tra_size > 2){
                tempvs = next_trajectory.at(tra_size * 0.5);
                std::tie(temp_index, std::ignore) = HeuristicIndex(heuristic_index, tempvs, circle_path);
                if (temp_index < 0)
                    continue;
            }
            ...
            if (!came_from.count(nextvs))
            {
                if (!cost_so_far.count(nextvs) || (new_cost < cost_so_far[nextvs]))
                {
                    ...
                    for (int i = 1; i <= 2; ++i)
                    {
                        if (current_index == circle_path.size() - 1){
                            id = current_index;
                        }else{
                            id = current_index + i;
